...
# Create a list of words to ignore (aka stop words)
...
class searcher:
    ...
    def query(self, q):
        ...
        for (score, urlid) in rankedscores[0:20]:
            print("{:f}\t{:s}\t{:s}".format(score, self.geturlname(urlid), urlid))

        return wordids, [r[1] for r in rankedscores[0:20]]
    ...
    def distancescore(self, rows):
        ...
        for row in rows:
            # We calculate the sum of the substractions between the word distances
    ...
    def inboundlinkscore(self, rows):
        ...
        inboundcount = dict([(url, self.con.execute('select count(*) from link where toid=%d' % url).fetchone()[0]) for url in uniqueurls])
